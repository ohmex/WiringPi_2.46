...
{
#ifdef OrangePiH6
	"IN", "OUT", "ALT1", "ALT2", "ALT3", "ALT4", "ALT5", "IOno"
#else
...
#else
	printf (" | %3d | %3d", physPinToGpio (physPin), physToWpiH6 [physPin]) ;
#endif

#ifndef OrangePiH6
	printf (" |    %s", physNames [physPin]) ;
#else
	printf (" | %s", physNamesH6 [physPin]) ;
#endif 
...
#else
    printf (" | %3d | %3d",  physToWpiH6 [physPin], physPinToGpio (physPin)) ;
#endif
...
void doReadall (void)
{
  ...
  /**/ if ((model == PI_MODEL_A) || (model == PI_MODEL_B) || (model == PI_MODEL_H6))
    abReadall (model, rev) ;
